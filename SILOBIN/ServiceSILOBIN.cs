...
{
    public partial class ServiceSILOBIN : ServiceBase
    {
        ...
        private void getErpDatabase()
        {
            try
            {
                ...
                String strSelect = "SELECT * from P_SILO_B_IN WHERE STATUS = '0' order by date_in";
        ...
        private bool setLocalhostDatabase(String strData, String strStatus, String strId)
        {
            ...
            {
                //将记录写入本地数据库
                bool addFlag = dbOperationLocalHost.Db_AddNew("INSERT INTO P_SILO_B_IN(DATA,DATE_IN,STATUS,ERPID) VALUES('" + strData + "',to_date('" + DateTime.Now.ToString() + "','YYYY-MM-DD HH24:MI:SS'),'0','"+strId+"')");
                if (addFlag)
                {
                    lm.ShowMsg("ERP数据： P_SILO_B_IN 表 ID=" + strId + " 记录写入本地数据库 P_SILO_B_IN 表 成功。");
                    return true;
                }
                else
                {
        ...
        private void updateLocalhostDatabase(String strId, String strDateOut, String strStatus)
        {
            try
            {
                String strUpdataSql = "UPDATE P_SILO_B_IN SET DATE_OUT = to_date('" + strDateOut + "','YYYY-MM-DD HH24:MI:SS'),STATUS='" + strStatus + "' WHERE ID='" + strId + "'";
                bool addFlag = dbOperationERP.DB_Update(strUpdataSql);
                if (addFlag)
                {
                    lm.ShowMsg("更新ERP数据库 P_SILO_B_IN 表 ID= " + strId + " 数据成功。");
                }
                else
                {
                    lm.ShowMsg("更新数据库失败。");
