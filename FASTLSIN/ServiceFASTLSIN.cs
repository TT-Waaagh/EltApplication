...
{
    public partial class ServiceFASTLSIN : ServiceBase
    {
        LogMsg lm = new LogMsg("D:\\EltApplication\\FASTLSINLog.txt");
        dbOperation dbOperationERP = new dbOperation("ERPDBconfig");
        dbOperation dbOperationLocalHost = new dbOperation("DBconfig");

        public ServiceFASTLSIN()
        {
            InitializeComponent();
        }
        ...
        private bool setLocalhostDatabase(String strData, String strStatus, String strId)
        {
            try
            {
                ...
                if (addFlag)
                {
                    lm.ShowMsg("ERP数据： P_FAST_LS_IN 表 ID=" + strId + " 记录写入本地数据库 P_FAST_LS_IN 表 成功。");
                    return true;
                }
                else
                {
        ...
        private void updateLocalhostDatabase(String strId, String strDateOut, String strStatus)
        {
            try
            {
                ...
                if (addFlag)
                {
                    lm.ShowMsg("更新ERP数据库 P_FAST_LS_IN 表 ID= " + strId + " 数据成功。");
                }
                else
                {
                    lm.ShowMsg("更新数据库失败。");
