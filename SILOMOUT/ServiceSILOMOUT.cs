...
{
    public partial class ServiceSILOMOUT : ServiceBase
    {
        LogMsg lm = new LogMsg("D:\\EltApplication\\SILOMOUTLog.txt");
        dbOperation dbOperationERP = new dbOperation("ERPDBconfig");
        dbOperation dbOperationLocalHost = new dbOperation("DBconfig");

        public ServiceSILOMOUT()
        {
            InitializeComponent();
        }
        ...
        private void getLocalhostDatabase()
        {
            try
            {
                ...
                String strSelect = "SELECT * from P_SILO_M_OUT WHERE STATUS = '0' order by date_in";
        ...
        private bool setERPDatabase(String strData, String strStatus, String strId)
        {
            ...
            {
                //将记录写入本地数据库
                bool addFlag = dbOperationERP.Db_AddNew("INSERT INTO P_SILO_M_OUT(DATA,DATE_IN,STATUS) VALUES('" + strData + "',to_date('" + DateTime.Now.ToString() + "','YYYY-MM-DD HH24:MI:SS'),'0')");
                if (addFlag)
                {
                    lm.ShowMsg("本地数据： P_SILO_M_OUT 表 ID=" + strId + " 记录写入ERP数据库 P_SILO_M_OUT 表 成功。");
                    return true;
                }
                else
                {
        ...
        private void updateERPDatabase(String strId, String strDateOut, String strStatus)
        {
            try
            {
                String strUpdataSql = "UPDATE P_SILO_M_OUT SET DATE_OUT = to_date('" + strDateOut + "','YYYY-MM-DD HH24:MI:SS'),STATUS='" + strStatus + "' WHERE ID='" + strId + "'";
                bool addFlag = dbOperationLocalHost.DB_Update(strUpdataSql);
                if (addFlag)
                {
                    lm.ShowMsg("更新本地数据库 P_SILO_M_OUT 表 ID= " + strId + " 数据成功。");
                }
                else
                {
                    lm.ShowMsg("更新数据库失败。");
